...
func climbingLeaderboard(ranked []int32, player []int32) []int32 {
	ranks := make([]int32, len(ranked))
	var currentRank int32 = 1
	for index, value := range ranked {
		if index > 0 && value != ranked[index-1] {
			currentRank++
		}
	...
	j := len(ranked) - 1
	for index, value := range player {
		for ; j > 0 && value >= ranked[j]; j-- {
		}
		if value < ranked[j] {
			playerRanks[index] = ranks[j] + 1
		} else {
			playerRanks[index] = ranks[j]
		}
	}
	return playerRanks
}
